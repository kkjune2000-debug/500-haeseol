...
def layer_content(bad, info):
    ...
    # ── 연습표에서 시키는 낱말은 그 파일 카드에 있어야 한다
    #    ★원 교재는 「놀라운 한국어 500문장」이다. 카드에 없는 낱말은 원 교재에도
    #    없는 것이라 2026-08-15에 연습표에서 뺐다(ㅡ·르·ㅂ·ㅅ 불규칙 14낱말).
    #    거꾸로 카드가 더 많은 것은 정상이다 — 믿다(regular)처럼 일부러 섞은
    #    대조용이 있어 불규칙 연습에는 넣지 않는다.
    #    ★「그 낱말이 파일에 두 번 나오는가」로 세면 안 된다 — AK_SND(음원 열쇠)에도
    #    낱말이 들어 있어, 카드가 없어도 두 번이 된다. 자료의 생김새로 가른다.
    #      연습표  ["덥다","be hot","더웠습니다",BI]              따옴표 셋
    #      카드    ["덥다","be hot","더웠습니다","…","…","…"]     따옴표 여섯
    CARD = re.compile(r'\["([^"]+)"(?:\s*,\s*"[^"]*"){4,}\s*\]')
    for f, s in SRC.items():
        m = re.search(r"pr80Build\(\[(.*?)\],\s*'", s, re.S)
        if not m:
            continue
        drill = re.findall(r'\["([^"]+)"', m.group(1))
        cards = set(CARD.findall(s))
        if not cards:
            continue                                  # 카드가 없는 파일은 견줄 것이 없다
        for w in drill:
            if w not in cards:
